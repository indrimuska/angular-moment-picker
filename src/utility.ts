...
export const valueToMoment = (
  ...
  if ($scope.model) {
    ...
    const views = $scope.views.all.slice(
      0,
      $scope.views.all.indexOf($scope.detectedMinView)
    );
...
export const setValue = (
  value: moment.Moment | Value,
  $scope: IDirectiveScopeInternal,
  $ctrl: IModelController,
  $attrs: ng.IAttributes
): void => {
  let modelValue = isValidMoment(value)
      ? (<moment.Moment>value).clone()
      : valueToMoment(<Value>value, $scope),
...
export const updateMoment = (
  ...
  if (!isValidMoment(model) || !value) model = value;
    ...
    if (!model.isSame(value)) {
      ...
      const views = $scope.views.all.slice(
        0,
        $scope.views.all.indexOf($scope.detectedMaxView) + 1
      );
      angular.forEach(views, (view: ViewString) => {
        const precision = $scope.views.precisions[view];
