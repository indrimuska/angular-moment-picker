...
export default class MonthView implements IView {
  ...
  public render(): string {
    let month: number = this.$scope.view.moment.month(),
      day: moment.Moment = this.$scope.view.moment
        .clone()
        .startOf('month')
        .startOf('week')
        .hour(12),
    ...
    for (let week = firstWeek; week <= lastWeek; week++)
      rows[week] = Array.apply(null, Array(this.perLine)).map(() => {
        ...
        let date = <IViewItem>{
          ...
          date: day.date(),
          class: [
            this.$scope.keyboard && day.isSame(this.$scope.view.moment, 'day')
              ? 'highlighted'
              : '',
            ...
            !selectable || day.month() != month
              ...
              : isValidMoment(this.$ctrl.$modelValue) &&
                day.isSame(this.$ctrl.$modelValue, 'day')
                ? 'selected'
                : '',
          ]
            .join(' ')
    ...
    angular.forEach(rows, (row: IViewItem[]) =>
      (<IViewItem[][]>this.rows).push(row)
    );
    // render headers
    this.headers = moment.weekdays().map((d: string, i: number) =>
      moment()
