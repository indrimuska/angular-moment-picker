...
export default class HourView implements IView {
  public perLine: number = this.provider.minutesPerLine;
  ...
  public render(): string {
    let i = 0,
      minute = this.$scope.view.moment
        .clone()
        .startOf('hour')
        .minute(this.provider.minutesStart),
      minutesFormat =
        this.provider.minutesFormat ||
        moment
    ...
    for (
      let m = 0;
      m <= this.provider.minutesEnd - this.provider.minutesStart;
      m += this.provider.minutesStep
    ) {
      ...
      this.rows[index].push(<IViewItem>{
        ...
        class: [
          ...
          !selectable
            ? 'disabled'
            : isValidMoment(this.$ctrl.$modelValue) &&
  ...
  public highlightClosest(): void {
    let minutes = <IViewItem[]>[],
      minute;
    angular.forEach(this.rows, (row) => {
      angular.forEach(row, (value) => {
        if (
          Math.abs(value.minute - this.$scope.view.moment.minute()) <
          this.provider.minutesStep
        )
          minutes.push(value);
      });
    });
    minute = minutes.sort((value1, value2) => {
      return Math.abs(value1.minute - this.$scope.view.moment.minute()) >
        Math.abs(value2.minute - this.$scope.view.moment.minute())
        ? 1
        : 0;
    })[0];
