...
export default class Directive implements ng.IDirective {
  ...
  public link = (
    $scope: IDirectiveScopeInternal,
    $element: ng.IAugmentedJQuery,
    $attrs: ng.IAttributes,
    $ctrl: IModelController,
    ...
    $transclude(($transElement: ng.IAugmentedJQuery) => {
      ...
      $scope.limits = {
        ...
        isAfterOrEqualMin: (
          ...
          return (
            !angular.isDefined($scope.limits.minDate) ||
        ...
        isBeforeOrEqualMax: (
          ...
          return (
            !angular.isDefined($scope.limits.maxDate) ||
        ...
        isSelectable: (
          ...
          return (
            $scope.limits.isAfterOrEqualMin(value, precision) &&
        ...
        checkValue: () => {
          ...
          if (!$scope.limits.isAfterOrEqualMin($ctrl.$modelValue))
            setValue($scope.limits.minDate, $scope, $ctrl, $attrs);
          if (!$scope.limits.isBeforeOrEqualMax($ctrl.$modelValue))
            setValue($scope.limits.maxDate, $scope, $ctrl, $attrs);
        },
        checkView: () => {
      ...
      $scope.views = {
        ...
        formats: {
          ...
          month:
            '[Dd]{1,4}(?![Ddo])|DDDo|[Dd]o|[Ww]{1,2}(?![Wwo])|[Ww]o|[Ee]|L{1,2}(?!T)|l{1,2}',
        ...
        detectMinMax: () => {
          ...
          if (!angular.isDefined(maxView))
            ...
            maxView = Math.min(
              $scope.views.all.length - 1,
              $scope.views.all.indexOf(maxView)
            );
      ...
      $scope.view = {
        ...
        update: () => {
          $scope.view.value = momentToValue($scope.view.moment, $scope.format);
        },
        toggle: () => {
        ...
        position: () => {
          ...
          let element = $element[0],
            ...
            shouldHaveClassTop =
              top + this.$window.pageYOffset - picker.offsetHeight > 0 &&
              top > winHeight / 2,
        ...
        keydown: (e) => {
          let view: IView = $scope.views[$scope.view.selected],
            precision = $scope.views.precisions[$scope.view.selected].replace(
              'date',
              'day'
            ),
            singleUnit = this.provider[precision + 'sStep'] || 1,
            operation =
              [KEYS.up, KEYS.left].indexOf(e.keyCode) >= 0 ? 'subtract' : 'add',
            highlight = (vertical?: boolean) => {
              let unitMultiplier = vertical ? view.perLine : 1,
                nextDate = $scope.view.moment
                  .clone()
                  [operation](singleUnit * unitMultiplier, precision);
              if (
                $scope.limits.isSelectable(
                  nextDate,
                  <moment.unitOfTime.StartOf>precision
                )
              ) {
        ...
        precision: () =>
          <moment.unitOfTime.DurationConstructor>$scope.view.selected.replace(
            'decade',
            'year'
          ),
        // header
        ...
        previous: {
          ...
          set: () => {
            if ($scope.view.previous.selectable) {
              $scope.view.moment.subtract(
                $scope.view.unit(),
                $scope.view.precision()
              );
              $scope.view.update();
              $scope.view.render();
            }
        ...
        setParentView: () => {
          $scope.view.change(
            $scope.views.all[
              Math.max(0, $scope.views.all.indexOf($scope.view.selected) - 1)
            ]
          );
        },
        // body
        render: () => {
          let momentPrevious = $scope.view.moment
              .clone()
              .startOf($scope.view.precision())
              .subtract($scope.view.unit(), $scope.view.precision()),
            momentNext = $scope.view.moment
              .clone()
              .endOf($scope.view.precision())
              .add($scope.view.unit(), $scope.view.precision());

          $scope.view.previous.selectable = $scope.limits.isAfterOrEqualMin(
            momentPrevious,
            $scope.view.precision()
          );
          $scope.view.previous.label = this.$sce.trustAsHtml(
            $scope.view.previous.selectable ? $scope.leftArrow : '&nbsp;'
          );
          $scope.view.next.selectable = $scope.limits.isBeforeOrEqualMax(
            momentNext,
            $scope.view.precision()
          );
          $scope.view.next.label = this.$sce.trustAsHtml(
            $scope.view.next.selectable ? $scope.rightArrow : '&nbsp;'
          );
      ...
      $scope.picker = angular.element(
        $element[0].querySelectorAll('.moment-picker')
      );
      $scope.container = angular.element(
        $scope.picker[0].querySelectorAll('.moment-picker-container')
      );
      $scope.input =
      ...
      ($scope.position || '')
        .split(' ')
        .forEach((className: string) => $scope.picker.addClass(className));
      if (!$scope.inline)
        $scope.picker[0].parentNode.removeChild($scope.picker[0]);
      else {
      ...
      this.$timeout(() => {
        angular.forEach(
          $scope.additions || {},
          (tempalteUrl: string, key: 'top' | 'bottom') => {
            let placeholder = angular.element(
              $scope.container[0].querySelector(
                '.moment-picker-addition.' + key
              )
            );
      ...
      this.$timeout(() => {
        if ($attrs['ngModel']) {
          ...
          if ($scope.value)
            $ctrl.$modelValue = valueToMoment($scope.value, $scope);
        }
        // view initialization
        if ($scope.startDate)
          $scope.view.moment = toMoment(
            $scope.startDate,
            $scope.format,
            $scope.locale
          );
        else if (isValidMoment($ctrl.$modelValue))
      ...
      if ($attrs['ngModel']) {
        $ctrl.$parsers.push(
          (viewValue) =>
            updateMoment(
              $ctrl.$modelValue,
              valueToMoment(viewValue, $scope),
              $scope
            ) || true
        );
        $ctrl.$formatters.push(
          (modelValue) => momentToValue(modelValue, $scope.format) || ''
        );
        $ctrl.$viewChangeListeners.push(() => {
          if ($attrs['ngModel'] != $attrs['momentPicker'])
            $scope.value = $ctrl.$viewValue;
        });
        $ctrl.$validators.minDate = (value) =>
          $scope.validate ||
          !isValidMoment(value) ||
          $scope.limits.isAfterOrEqualMin(value);
        $ctrl.$validators.maxDate = (value) =>
          $scope.validate ||
          !isValidMoment(value) ||
      ...
      $scope.$watch(
        () => momentToValue($ctrl.$modelValue, $scope.format),
        (newViewValue, oldViewValue) => {
          ...
          $scope.view.moment = (
            newModelValue || moment().locale($scope.locale)
          ).clone();
          $scope.view.update();
          $scope.view.render();
      ...
      $scope.$watch(
        () => $ctrl.$modelValue && $ctrl.$modelValue.valueOf(),
        () => {
          let viewMoment = (isValidMoment($ctrl.$modelValue)
            ? $ctrl.$modelValue
      ...
      $scope.$watch(
        () => toValue($scope.startDate, $scope.format, $scope.locale),
        (newViewValue, oldViewValue) => {
          if (newViewValue == oldViewValue) return;
      ...
      $scope.$watch('locale', (locale: string, previous: string) => {
        ...
        if (isValidMoment($ctrl.$modelValue))
          setValue($ctrl.$modelValue.locale(locale), $scope, $ctrl, $attrs);
        if (isValidMoment($scope.view.moment))
          $scope.view.moment = $scope.view.moment.locale(locale);
        if (isValidMoment($scope.limits.minDate))
          $scope.limits.minDate = $scope.limits.minDate.locale(locale);
        if (isValidMoment($scope.limits.maxDate))
      ...
      $scope.input
        ...
        .on('keydown', (e) => {
          if ($scope.keyboard) $scope.view.keydown(e);
        });
